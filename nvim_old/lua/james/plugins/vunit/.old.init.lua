...
local find_test_line = function(sv_bufnr, name)
  local query = vim.treesitter.query.parse("systemverilog", string.format(test_function_query_string, name))
  local parser = vim.treesitter.get_parser(sv_bufnr, "systemverilog", {})
  local tree = parser:parse()[1]
  local root = tree:root()

  for id, node, metadata in query:iter_captures(root, sv_bufnr, 0, -1) do
    if id == 1 then
...
local attach_to_buffer = function(sv_bufnr, output_bufnr, command)
  vim.api.nvim_create_autocmd("BufWritePost", {
    group = vim.api.nvim_create_augroup(string.format("teej-automagic-%s", sv_bufnr), { clear = true }),
    pattern = "*.sv",
    callback = function()
      local sv_win = vim.api.nvim_get_current_win()
      vim.api.nvim_buf_clear_namespace(sv_bufnr, ns, 0, -1)
      ...
      vim.fn.jobstart(command, {
        ...
        on_exit = function()
          ...
          for key, test in pairs(state.tests) do
            local test_line = find_test_line(sv_bufnr, test.name)
            if test_line then
              local text
              ...
              vim.api.nvim_buf_set_extmark(sv_bufnr, ns, test_line, 0, {
                virt_text = { text },
              })
            end
          ...
          -- vim.fn.setloclist(sv_win, failed, "r")
          -- if #failed > 0 then
          --   vim.cmd.lopen()
          --   vim.cmd.lfirst()
          -- end
          vim.diagnostic.set(ns, sv_bufnr, failed, {})
        end,
      })
    end,
  })
...
attach_to_buffer(1, 6, { "python3", "run.py", "--export-json" })
