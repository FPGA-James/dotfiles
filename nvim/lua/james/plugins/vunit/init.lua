...
local find_test_line = function(sv_bufnr, name)
  ...
  local query = vim.treesitter.query.parse("systemverilog", formatted)
  local parser = vim.treesitter.get_parser(sv_bufnr, "systemverilog", {})
  local tree = parser:parse()[1]
  local root = tree:root()

  for id, node in query:iter_captures(root, sv_bufnr, 0, -1) do
    if id == 1 then
...
local add_sv_test = function(state, entry)
...
local attach_to_buffer = function(bufnr, command)
  ...
  vim.api.nvim_buf_create_user_command(bufnr, "SVTestLineDiag", function()
...
vim.api.nvim_create_user_command("GoTestOnSave", function()
  attach_to_buffer(vim.api.nvim_get_current_buf(), { "python3", "run.py", "--export-json" })
end, {})
